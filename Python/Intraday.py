...
def createMasterDictionary(Exchange, printTicker=True, saveNewData=True):
    '''
    Iterates through every stock in the Exchange list and adds it to a dictionary with the value of its price list
    saveNewData set to True will search and save the data to get the most recent result
    saveNewData set to False will use all of the already saved data.  Saves a lot of time is data is recent.
    '''
    masterDict = {}
    if len(Exchange) > 1:
        Exchange = Exchange[1:]
...
if __name__ == '__main__':
    ...
    def chooseExchangeList(Exchange):
        ...
        return(createMasterDictionary(exchange, saveNewData=False))
    
    print(chooseExchangeList("15BelowNASDAQ"))
